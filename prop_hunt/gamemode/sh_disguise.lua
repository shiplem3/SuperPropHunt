...
function PlayerMeta:CanDisguiseAsPropNonTake(ent)
...
function PlayerMeta:GetPropEyePos()
	...
	trace.endpos = trace.start + Vector(0, 0, reach) + (angles:Forward() * -90)
	local tab = ents.FindByClass("prop_ragdoll")
	table.insert(tab, self)
	trace.filter = tab

	local tr = util.TraceLine(trace)
	return trace.start + (trace.endpos - trace.start):GetNormal() * reach
end
