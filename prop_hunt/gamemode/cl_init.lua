...
function GM:CalcView(ply, pos, angles, fov)
	...
	if IsValid(ply) then
		...
		if ply:IsPlayer() && !ply:IsDisguised() && ply:Team() == 3  then
			...
			trace.endpos = trace.start + angles:Forward() * -(reach + 60)
			local tab = ents.FindByClass("prop_ragdoll")
			table.insert(tab, ply)
			trace.filter = tab

			local tr = util.TraceLine(trace)
			view.origin = trace.start + (trace.endpos - trace.start):GetNormal() * math.Clamp(trace.start:Distance(tr.HitPos) - 5, 0, reach*60)
		...
		if ply:IsPlayer() && ply:IsDisguised() then
			...
			local trace = {}
			trace.start = ply:GetPropEyePos()
			trace.endpos = trace.start + angles:Forward() * -(reach + 55)
			local tab = ents.FindByClass("prop_ragdoll")
			table.insert(tab, ply)
			trace.filter = tab

			local tr = util.TraceLine(trace)
			view.origin = trace.start + (trace.endpos - trace.start):GetNormal() * math.Clamp(trace.start:Distance(tr.HitPos) - 5, 0, reach*40)
